...
export function ModernTech({ data }: Props) {
  return (
    <div className="w-full h-full bg-white text-slate-800 p-8 grid grid-cols-12 gap-8 min-h-[1056px]">
      ...
      <div className="col-span-4 flex flex-col gap-6 border-r border-slate-200 pr-6">
        <div className="flex flex-col gap-1">
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 leading-tight uppercase font-grotesk">
        ...
        <div className="mt-6">
          <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-3 border-b border-slate-100 pb-1">
            Skills
          </h3>
          <div className="flex flex-wrap gap-2">
            {data.skills.map((skill, i) => (
              <span
                key={i}
                className="bg-slate-100 text-slate-700 px-2 py-1 rounded text-[10px] font-medium border border-slate-200"
              >
                {skill}
              </span>
            ))}
