...
function SectionHeader({ title, isOpen, toggle }: SectionHeaderProps) {
  return (
    <button
      onClick={toggle}
...
export function Editor({ data, onChange }: Props) {
  ...
  const { isPolishEnabled, isGenerateEnabled } = FEATURE_FLAGS;
  ...
  const handleMagicSummary = async () => {
    ...
    const summary = await generateSummary(data.personalInfo.role, data.skills);
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-end">
                    ...
                    {(isGenerateEnabled || isPolishEnabled) && (
                      <div className="flex gap-2">
                        ...
                        {isPolishEnabled && (
                          <button
                            onClick={() =>
                              handlePolish('summary', data.personalInfo.summary, 'summary', (v) =>
      ...
      <div className="mb-4 rounded-xl border border-white/10 overflow-hidden bg-zinc-900/30">
        <div className="flex items-center bg-zinc-900/50 border-b border-white/5 pr-4">
          <SectionHeader
            ...
            toggle={() => toggle('experience')}
          />
          <button onClick={addExperience} className="p-1 hover:bg-white/10 rounded">
                ...
                {data.experience.map((exp) => (
                  <div key={exp.id} className="relative pl-4 border-l border-white/10">
                    <button
                      ...
                      className="absolute -left-2.5 top-0 bg-zinc-900 p-1 rounded-full text-red-400 hover:text-red-300 border border-white/10"
                    >
                      <Trash2 size={12} />
                    </button>
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between items-end">
                        ...
                        {isPolishEnabled && (
                          <button
                            onClick={() =>
                              handlePolish(exp.id, exp.description, 'bullet', (v) =>
